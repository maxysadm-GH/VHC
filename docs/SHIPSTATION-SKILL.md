...
> **NOTE:** All credentials are stored in environment variables or Azure Key Vault.
> See `.env.example` for required variables. Never commit actual credentials.

### ShipStation Store IDs (Not secrets - safe to reference)
| Store ID | Name | Use |
|----------|------|-----|
...
```bash
# ShipStation
SHIPSTATION_API_KEY=<from-azure-keyvault>
SHIPSTATION_API_SECRET=<from-azure-keyvault>
SHIPSTATION_X_PARTNER=<from-azure-keyvault>

# Shopify
SHOPIFY_ADMIN_TOKEN=<from-azure-keyvault>
SHOPIFY_STOREFRONT_TOKEN=<from-azure-keyvault>

# Weather API
WEATHER_API_KEY=<from-azure-keyvault>
```
...
```json
"headers": {
    "Authorization": "Basic <BASE64_ENCODED_API_KEY:SECRET>",
    "x-partner": "<X_PARTNER_KEY>"
}
```
...
$headers = @{
    ...
    "x-partner" = $env:SHIPSTATION_X_PARTNER
}
        ...
        "actions": {
            "Get_Orders": {
                ...
                "inputs": {
                    ...
                    "method": "GET",
                    "headers": {
                        "Authorization": "Basic @{parameters('shipstation_auth')}",
                        "x-partner": "@{parameters('shipstation_x_partner')}"
                    }
                }
            },
            "Set_TotalPages": {
...
$xPartnerKey = $env:SHIPSTATION_X_PARTNER
...
**Solution:** Add `x-partner` header (see env vars)
...
```powershell
$auth = [Convert]::ToBase64String([Text.Encoding]::ASCII.GetBytes("$($env:SHIPSTATION_API_KEY):$($env:SHIPSTATION_API_SECRET)"))
$headers = @{
    "Authorization" = "Basic $auth"
    "x-partner" = $env:SHIPSTATION_X_PARTNER
    "Content-Type" = "application/json"
